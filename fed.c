/*
 *      fed.c -- fed decoder and packet dump
 *
 *      Copyright (C) 2007
 *
...
bool fed_justhex = false;

/* ---------------------------------------------------------------------- */

static void fed_disp_service_id(uint8_t service_id)
{
...
static void fed_disp_state_id(uint8_t state_id, uint8_t loc_id)
{
...
static void fed_disp_loc_id(uint8_t loc_id)
{
...
static void fed_disp_vehicle_id(uint16_t vehicle_id)
{
...
static void fed_disp_state(uint8_t state, uint8_t direction)
{
...
static void fed_disp_direction(uint8_t direction)
{
    if (direction)
    {
...
static void fed_disp_shortinfo(uint8_t short_info)
{
...
static void fed_print_crc(char crc[7]) {
  int i;

  verbprintf(2, "fed CRC:");
...
void fed_print_message_hex(uint64_t message)
{
    verbprintf(2, "fed MESSAGE HIGH: %08x\n", message >> 32);
    verbprintf(2, "fed MESSAGE  LOW: %08x\n", message);
}


static char fed_is_crc_correct(uint64_t message)
{
    int i;
    char crc[7];
    char doinvert;
...
/*
 *  As specified in http://www.lfs-bw.de/Fachthemen/Digitalfunk-Funk/Documents/Pruefstelle/TRBOS-fed.pdf
 */
static void fed_disp_packet(uint64_t message)
{
    ...
    fed_print_message_hex(message);

    verbprintf(0, "fed: %08x%04x", message >> 32, ((uint32_t)message >> 16));

    if(!fed_justhex)
    {
        verbprintf(0, " (");
        service_id = (message >> 16) & 0xF;
        fed_disp_service_id(service_id);

        state_id = (message >> 20) & 0xF;
        loc_id = (message >> 24) & 0xFF;
        fed_disp_state_id(state_id, loc_id);
        fed_disp_loc_id(loc_id);

        vehicle_id = (message >> 32) & 0xFFFF;
        fed_disp_vehicle_id(vehicle_id);

        state = (message >> 48) & 0xF;

        //model = (message >> 52) & 0x1;
        direction = (message >> 53) & 0x1;
        fed_disp_state(state, direction);

        fed_disp_direction(direction);

        short_info = (message >> 54) & 0x3;
        fed_disp_shortinfo(short_info);
...
void fed_init(struct demod_state *s)
{
    memset(&s->l2.uart, 0, sizeof(s->l2.uart));
    s->l2.fedfsk.rxstate = 0;
    s->l2.fedfsk.rxbitstream = 0;
    s->l2.fedfsk.rxbitcount = 0;
}

/* ---------------------------------------------------------------------- */

void fed_rxbit(struct demod_state *s, int bit)
{
    uint64_t msg;
    int i;
    ...
    s->l2.fedfsk.rxstate = ((s->l2.fedfsk.rxstate << 1) & 0xFFFFFFFE) | bit;

    // Check if the sync pattern is in the buffer
    if ((s->l2.fedfsk.rxstate & 0xFFFFFFFF) == 0xAABAABAA)
    {
        verbprintf(1, "fed ->SYNC<-\n");
        s->l2.fedfsk.rxbitstream = 0; // reset RX buffer
        s->l2.fedfsk.rxbitcount = 1;  // > 1 means we have a valid SYNC
    }

    // If we have a valid SYNC, record the message by appending it (from the left) to the RX buffer
    else if (s->l2.fedfsk.rxbitcount >= 1) {
        verbprintf(4, "fed BIT: %1x\n", bit);

        s->l2.fedfsk.rxbitstream = (s->l2.fedfsk.rxbitstream >> 1) | ((uint64_t) bit << 63);
        s->l2.fedfsk.rxbitcount++;

        // Wait until message has been completely received. If so, decode and display it and reset
        if (s->l2.fedfsk.rxbitcount == 49)
        {
            // If the CRC check fails, try to fix a one bit error
            if (!fed_is_crc_correct(s->l2.fedfsk.rxbitstream))
            {
                i = 0;
                msg = s->l2.fedfsk.rxbitstream;
                while (i <= 47)
                {
                    if (fed_is_crc_correct(msg ^ (1 << (i+16))))
                    {
                        verbprintf(2, "fed was able to correct a one bit error by swapping bit %d Original packet:\n", i);
                        fed_disp_packet(s->l2.fedfsk.rxbitstream);
                        s->l2.fedfsk.rxbitstream = (msg ^ (1 << (i+16))) | 1; // lowest bit set means that the CRC has been corrected by us
                        break;
                    }
                    i++;
                }
                if (i == 48)
                {
                    verbprintf(2, "fed: unable to correct CRC error\n");
                }
            }

            fed_disp_packet(s->l2.fedfsk.rxbitstream);
            s->l2.fedfsk.rxbitcount = 0; // Reset counter, meaning "no valid SYNC yet"
            s->l2.fedfsk.rxstate = 0;    // Reset message input buffer
        }
    }
}
